...
### 需求:最小 QA CLI 输出
系统必须提供命令 `python -m app.qa --q "<question>" --mode dense|bm25|hybrid`。除既有字段外，系统还必须在运行输出中记录：`calibrated_query`、`calibration_reason`、`query_retry_used`、`query_retry_reason`（若触发），并且必须输出 `answer_citations` 与 `output_warnings`。
