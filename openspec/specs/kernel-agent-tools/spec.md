# kernel-agent-tools 规范

## 目的
定义 Python kernel 内 agent tool registry、统一 call/result/failure envelope、evidence policy 与流式声明约束，使 planner runtime 能够以稳定契约接入本地能力并保留可审计执行语义。
