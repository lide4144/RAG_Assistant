...
## 目的
定义 Node Gateway 作为前端统一入口时的聊天、任务与协议编排边界，确保前端可通过单一网关接入 kernel、web 能力以及 agent-first planner runtime。
...
- **那么** Gateway 必须继续使用现有链路，而不是在第一阶段强制切到 planner runtime
...
网关必须输出统一事件协议，至少包含 `message`、`sources`、`messageEnd`、`error` 四类聊天事件，并支持任务型事件（如任务状态与进度）与聊天事件并行共存；在 agent-first 架构下，Gateway 禁止直接暴露 planner runtime 内部节点、tool 细节或非稳定执行事件给当前前端协议层，除非后续专门变更重新定义协议。
