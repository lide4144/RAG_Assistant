...
#### 场景:降级事件携带受控原因
- **当** runtime 触发 planner fallback、tool fallback 或 legacy fallback
...
#### 场景:降级后仍返回回答
- **当** planner runtime 因置信不足而触发 legacy fallback，但兼容路径仍成功返回回答
- **那么** Gateway 必须先输出 `fallback` 事件，再继续输出标准聊天事件，并最终以 `messageEnd` 结束，而不是直接输出 `error`
