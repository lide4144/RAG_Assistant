...
### 需求:系统必须支持受限的多动作计划执行
系统必须支持受限的顺序执行计划，允许的动作仅包括 `catalog_lookup`、`cross_doc_summary`、`fact_qa`、`control`、`paper_assistant`；计划步数必须有硬上限，且每个后续步骤必须显式声明对前序产物的依赖。
...
系统必须在运行 trace 中输出 planner runtime、tool 调用与执行链路观测字段，至少包含 `planner_used`、`planner_source`、`planner_confidence`、`primary_capability`、`strictness`、`action_plan`、`selected_path`、`execution_trace`、`short_circuit`、`truncated`、`planner_fallback`、`planner_fallback_reason`，并能够区分 planner 级与 tool/pipeline 级降级；当触发回退时必须记录回退原因与停止点。
