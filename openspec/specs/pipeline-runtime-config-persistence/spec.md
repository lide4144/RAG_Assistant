...
### 需求:系统必须提供运行态概览聚合输出
系统必须提供统一运行态概览输出，聚合 LLM stage 与 pipeline runtime 配置，并给出状态等级（`READY`/`DEGRADED`/`BLOCKED`/`ERROR`）与原因列表。对于 Marker pipeline，概览输出还必须包含 `use_llm`、`llm_service`、最近一次降级状态、产物健康摘要与可直接渲染的 UI 文案。
