...
(
  cd "$ROOT_DIR/gateway"
  GATEWAY_HOST="$GATEWAY_HOST" GATEWAY_PORT="$GATEWAY_PORT" KERNEL_BASE_URL="http://$KERNEL_HOST:$KERNEL_PORT" "${GATEWAY_CMD[@]}"
) &
GATEWAY_PID=$!
...
(
  ...
  PORT="$FRONTEND_PORT" NEXT_PUBLIC_KERNEL_BASE_URL="http://$KERNEL_HOST:$KERNEL_PORT" "${FRONTEND_CMD[@]}"
) &
FRONTEND_PID=$!
